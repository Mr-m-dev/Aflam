...
export default function TVDetailScreen() {
  ...
  const handleFavorite = async () => {
    ...
    await toggleFavorite({
      id: tvId,
      type: "tv",
      title: (show.name || show.original_name) as string,
      poster_path: show.poster_path as string | null,
      vote_average: show.vote_average as number,
  ...
  const handleTrailer = () => {
    const media = show?.media as { trailers?: { key: string }[] } | undefined;
  ...
  if (isLoading) {
    return <View style={styles.loadingContainer}><View style={styles.loadingSkeleton} /></View>;
  }
  ...
  const backdrop = backdropUrl(show.backdrop_path as string | null, "w1280");
  const poster = posterUrl(show.poster_path as string | null, "w342");
  const genres = (show.genres as { id: number; name: string }[]) || [];
  const credits = show.credits as { cast?: Record<string, unknown>[]; crew?: Record<string, unknown>[] } | undefined;
  ...
  const seasons = (show.seasons as Record<string, unknown>[]) || [];
  const similar = ((show.similar_tvshows || []) as Record<string, unknown>[]).slice(0, 10);
  const media = show.media as { trailers?: { key: string }[] } | undefined;
    ...
    <View style={styles.screen}>
      ...
      <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.content}>
          <Text style={styles.movieTitle}>{(show.name || show.original_name) as string}</Text>

          <View style={styles.metaRow}>
            {show.vote_average ? (
              <View style={styles.ratingChip}>
                <Ionicons name="star" size={13} color={Colors.starYellow} />
                <Text style={styles.ratingText}>{(show.vote_average as number).toFixed(1)}</Text>
              </View>
            ) : null}
            {year ? <Text style={styles.metaText}>{year}</Text> : null}
            {show.number_of_seasons ? (
              <Text style={styles.metaText}>{`${show.number_of_seasons} Season${(show.number_of_seasons as number) > 1 ? "s" : ""}`}</Text>
            ) : null}
          </View>
          ...
          {show.overview ? (
            <View style={styles.section}>
              ...
              <Text style={styles.overview}>{show.overview as string}</Text>
            </View>
          ) : null}
              ...
              {seasons.map((season) => (
                <Pressable
                  ...
                  onPress={() => setExpandedSeason(
                    expandedSeason === (season.season_number as number)
                      ? null
                ...
                >
                  <View style={styles.seasonHeader}>
                    <View style={styles.seasonInfo}>
                      {season.poster_path ? (
                        <Image
                          source={{ uri: posterUrl(season.poster_path as string | null, "w185") ?? "" }}
                          style={styles.seasonPoster}
                          contentFit="cover"
                        />
                      ) : null}
                      <View style={styles.seasonText}>
                        <Text style={styles.seasonName}>{season.name as string}</Text>
                        <Text style={styles.seasonEps}>
                          {`${season.episode_count} episodes`}
                          {season.air_date ? ` · ${(season.air_date as string).slice(0, 4)}` : ""}
                        </Text>
                      </View>
                    </View>
                    <Ionicons
                  ...
                  {expandedSeason === season.season_number && season.overview ? (
                    <Text style={styles.seasonOverview}>{season.overview as string}</Text>
                  ) : null}
                </Pressable>
              ))}
            </View>
          ...
          {cast.length > 0 && (
            <View style={styles.section}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <CastCard id={item.id} name={item.name} character={item.character} profile_path={item.profile_path} />
                )}
          ...
          {similar.length > 0 && (
            <View style={styles.section}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <PosterCard item={item as unknown as import("@/lib/tmdb").TMDBTVShow} type="tv" width={110} height={165} />
                )}
                showsHorizontalScrollIndicator={false}
              />
            </View>
...
const styles = StyleSheet.create({
  ...
  posterOverlay: { position: "absolute", bottom: -60, left: 20, shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.6, shadowRadius: 16, elevation: 10 },
  poster: { width: 120, height: 180, borderRadius: 10, overflow: "hidden" },
  posterPlaceholder: { backgroundColor: Colors.surface, alignItems: "center", justifyContent: "center" },
  content: { paddingTop: 80, paddingHorizontal: 20 },
  movieTitle: { color: Colors.textPrimary, fontSize: 26, fontFamily: "Inter_700Bold", lineHeight: 32, marginBottom: 10 },
  ...
  genreChip: { borderWidth: 1, borderColor: Colors.border, borderRadius: 16, paddingHorizontal: 12, paddingVertical: 5, marginRight: 8 },
  ...
  trailerBtn: { flex: 1, flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: Colors.accent, borderRadius: 8, paddingVertical: 12, gap: 6 },
  trailerBtnText: { color: Colors.white, fontSize: 14, fontFamily: "Inter_700Bold" },
  favBtn: { flexDirection: "row", alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: Colors.border, borderRadius: 8, paddingVertical: 12, paddingHorizontal: 20, gap: 6 },
  ...
  sectionTitle: { color: Colors.textPrimary, fontSize: 18, fontFamily: "Inter_700Bold", marginBottom: 14 },
  overview: { color: Colors.textSecondary, fontSize: 14, fontFamily: "Inter_400Regular", lineHeight: 22 },
  seasonItem: { backgroundColor: Colors.surface, borderRadius: 10, padding: 14, marginBottom: 8, borderWidth: 1, borderColor: Colors.border },
  seasonHeader: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  seasonInfo: { flexDirection: "row", alignItems: "center", gap: 12, flex: 1 },
  seasonPoster: { width: 40, height: 60, borderRadius: 4 },
  seasonText: { flex: 1 },
  seasonName: { color: Colors.textPrimary, fontSize: 14, fontFamily: "Inter_600SemiBold" },
  seasonEps: { color: Colors.textMuted, fontSize: 12, fontFamily: "Inter_400Regular", marginTop: 2 },
  seasonOverview: { color: Colors.textMuted, fontSize: 12, fontFamily: "Inter_400Regular", lineHeight: 18, marginTop: 10 },
});
